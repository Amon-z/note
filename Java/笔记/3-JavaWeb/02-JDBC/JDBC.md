...
![image-20231107194645433](./assets/image-20231107194645433.png)
...
![image-20231107194839117](./assets/image-20231107194839117.png)
...
![image-20231107195148343](./assets/image-20231107195148343.png)

- 导入驱动包，将mysql的驱动包放在模块下的lib目录（随意命名）下，并将jar包添加为库文件，在添加为库文件的时候有如下三个选项：`Global Library`全局有效，`Project Library`项目有效， `Module Library`模块有效
...
```java
static void    registerDriver(Driver driver)    使用DriverManager注册给的那个的驱动程序
```

registerDriver方法是用于注册驱动的，但是前面我们不是这样写的。而是如下实现：

```sql
Class.forName("com.mysql.jdbc.Driver");
```

MySQL提供的Driver类的实现源码如下：
...
PreparedStatement  prepareStatement(sql)  // 预编译SQL的执行SQL对象，可防止SQL注入
    
CallableStatement prepareCall(sql)  // 执行存储过程的对象，存储过程在MySQL中不常用，不进行讲解
```
...
```java
void setAutoCommit(boolean autoCommit)    // 将此连接的自动提交模式设置为给定状态，true表示自动提交事务，false表示手动提交事务。而开启事务需要将该参数设为为false。
void commit()     // 使自上次提交/回滚以来所做的所有更改成为永久，并释放此Connection对象当前持有的所有数据库锁。 
    
void rollback()   // 撤销当前事务中所做的所有更改，并释放此Connection对象当前持有的所有数据库锁。
```
...
* 需求：查询account账户表数据，封装为Account对象中，并且存储到ArrayList集合中
  ...
  ![image-20231107225058257](./assets/image-20231107225058257.png)
...
![image-20231107225231595](./assets/image-20231107225231595.png)

此时我们就能在数据库中看到user表

![image-20231107225236491](./assets/image-20231107225236491.png)

接下来在浏览器的地址栏输入 `localhost:8080/login.html` 就能看到如下页面

![image-20231107225242628](./assets/image-20231107225242628.png)

我们就可以在如上图中输入用户名和密码进行登陆。用户名和密码输入正确就登陆成功，跳转到首页。用户名和密码输入错误则给出错误提示，如下图

![image-20231107225248559](./assets/image-20231107225248559.png)

但是我可以通过输入一些特殊的字符登陆到首页。

用户名随意写，密码写成 `' or '1' ='1`

![image-20231107225303140](./assets/image-20231107225303140.png)
...
![image-20231107225323831](./assets/image-20231107225323831.png)
...
* 执行SQL语句，查看 `D:\mysql.log` 日志如下:
  ...
  ![image-20231107225345510](./assets/image-20231107225345510.png)
